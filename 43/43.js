  ...
  var i = (() => {
    class i {
          ...
          ? Laya.timer.currTimer - this.lasterrortime > 100 &&
            (this.connect_state == e.tryconnect
              ...
              ? GameMgr.Inst.logined
                ...
                : (this.open_func = null)
              : this.connect_state == e.reconnecting && this._try_to_linknext())
          : i == Laya.Event.OPEN &&
            (this.connect_state == e.tryconnect
              ? (this.open_func = null)
      ...
      _Reconnect() {
        ...
        Laya.timer.once(this.reconnect_span[this.reconnect_count], this, () => {
          app.Log.log(`LobbyNetMgr _Reconnect  count:${t.reconnect_count}`);

          t.connect_state == e.reconnecting && t._fetch_gateway();
        });
      ...
      refreshCommonShow() {
        uiscript.UI_Common.Inst.label_net_lobby.text = `未连接:${this.servername}`;

        uiscript.UI_Common.Inst.label_net_lobby.text = `尝试连接:${this.servername}`;

        uiscript.UI_Common.Inst.label_net_lobby.text = `大厅服务器:${this.servername}`;

        uiscript.UI_Common.Inst.label_net_lobby.text = `大厅服务器:${this.servername} 断开连接`;

        uiscript.UI_Common.Inst.label_net_lobby.text = `大厅服务器:${this.servername} 正在重连`;
      ...
      _fetch_gateway() {
        ...
        app.Log.log(
          `${i.gateway_url} ${i.gateway_region_name} ${JSON.stringify(
            i.gateway_regions
          )}`
        );
