...
!(t => {
  const e = (() => {
    ...
    (e.prototype.render_item = function({index, container, cache_data}) {
      ...
      const l = a.getChildByName('btn_detail');
      const h = l.getChildByName('content').getChildByName('num');

      const c = () => {
        let t = s.count;
        if (i.during_sell)
          if (o.can_sell) {
            const e = i.choose_count[s.item_id];
            null == e || 0 == e
      ...
      const u = l.getChildByName('content');
      this.during_sell
    ...
    (e.prototype.on_btn_sell = function() {
      const e = [];
      for (const i in this.choose_count)
    ...
    (e.prototype.clear_choose = function() {
      for (const t in this.choose_count) {
