  ...
  const e = (() => {
    function e(e, i) {
      ...
      if (
        ...
        for (let a = 0; a < e.length; a++) this._registerService(e[a]);
    }
    ...
    (e.prototype._handleMsg = function(e) {
      ...
      switch (i.type) {
        ...
        case t.HeaderType.RESPONSE:
          const n = t.MessageWrapper.decodeRpc(e);
          return this.requestClientHandle_.emitResponse(i.reqIndex, n.data);
        case t.HeaderType.NOTIFY:
          const a = t.MessageWrapper.decodeMessage(e);
          const r = this.handler_[a.$type.fullName];
          if (!r)
    ...
    (e.prototype._registerService = function(e) {
      const i = this;
      const n = t.ProtobufManager.lookupService(`lq.${e}`);
      if (!n) throw new Error(`ERR_SERVICE_NOT_FOUND, name=${e}`);
      const a = n.create(({fullName}, e, n) => {
        i._requestMessage(fullName, e, n);
      });
      this.services_[e] = a;
    }),
    (e.prototype._sendMessage = function({header, packet}) {
      if (!this.socket_) throw new Error('ERR_SOCKET_NOT_CONNECT');
      const i = t.MessageWrapper.encodeHeaderData(header);
      const n = t.MessageWrapper.encodeMessage(packet);
    ...
    (e.prototype._sendRpc = function(e, {header, packet}) {
      if (!this.socket_) throw new Error('ERR_SOCKET_NOT_CONNECT');
      const n = t.MessageWrapper.encodeHeaderData(header);
      const a = t.MessageWrapper.encodeRpc(e, packet);
