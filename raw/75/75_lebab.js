...
!(t => {
  const e = (e => {
              ...
              : (cfg.audio.audio.forEach(({type, path}) => {
                ...
                t.LoadMgr.loadRes(
                  ...
                  Laya.Handler.create(n, () => Laya.timer.frameOnce(1, n, () => {
                    ...
                    const a = n.desktop.getChildByName('main_camera');
                    GameMgr.Inst.addScene(n.desktop);
                    const r = n.desktop.addChild(new Laya.DirectionLight());
                    ...
                    const s = Laya.loader.getRes(n.scene_path2);
    ...
    (i.prototype.active_common_texture2d = (e, i) => {
      i.runWith(0);
      for (
        var n = [], a = 0;
      ...
      const s = () => {
        ...
        const t = Laya.loader.getRes(n[r]);
        t && t.recreateResource(), r++, s();
      };

      s();
    }),
    (i.prototype.clearTexture_commont_texture2d = () => {
      for (
        let e = t.EffectMgr.d3res_map.mj_common_texture2d, i = 0;
        i < e.length;
        i++
      ) {
    ...
    (i.prototype._load_my_mjp_view = function(e, i) {
      ...
      const a = t.GameUtility.get_common_view_res_name(t.ECommonView.mjp);
    ...
    (i.prototype._load_my_desktop_view = function(e, i) {
      const n = this;
      let a = t.GameUtility.get_common_view_res_name(t.ECommonView.desktop);
    ...
    (i.prototype._load_player_views = function(e, i, n) {
      ...
      for (var s = {}, o = {}, l = {}, h = 0; h < e.length; h++) {
        ...
        if (c && c.character) {
          const u = c.character;
          let _ = 'scene/effect_hupai_default.lh';
          let d = 'scene/hand_human.lh';
          const f = cfg.item_definition.character.get(u.charid);
          f && (d = `scene/${f.hand}.lh`);
          let p = '';
          let m = 'scene/liqi_default.lh';
          let g = '';
          if (u.views)
            for (let y = 0; y < u.views.length; y++) {
              const v = u.views[y].slot;
              const b = u.views[y].item_id;
              if (v == t.EPlayerView.liqi_bgm) {
                const w = cfg.item_definition.item.get(b);
                w && (g = w.sargs[0]);
              } else {
                const x = cfg.item_definition.view.get(b);
                x &&
          ...
          (r[_] = 1),
            ...
            '' != g && (l[g] = 1);
          const I = u.skin;
          const C = cfg.item_definition.skin.get(I);
          (o[`${C.path}/full.png`] = 1),
            (o[`${C.path}/half.png`] = 1),
            C.no_reverse &&
                ...
                t.EffectMgr.preheat_3d_effect(
                  ...
                  Laya.Handler.create(a, () => {
                    t.EffectMgr.preheat_3d_effect(
                      ...
                      !0,
                      Laya.Handler.create(a, () => {
                        Laya.timer.frameOnce(5, a, () => {
                          i.run();
                          for (const t in l) Laya.loader.load(`audio/${t}`);
                        });
                      }),
                      Laya.Handler.create(
                        a,
                        t => n.runWith(0.65 + 0.35 * t),
                        null,
                        !1
                      )
                    );
    ...
    (i.prototype._on_quit = function() {
      if (
        ...
        for (i = 0; i < this._model_list.length; i++) {
          const e = this._model_list[i].url;
